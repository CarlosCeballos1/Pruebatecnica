...
export function TaskCard({ task, onUpdate }: TaskCardProps) {
  const { attributes, listeners, setNodeRef, transform, setActivatorNodeRef } = useDraggable({ id: task.id });
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="task-card bg-white/15 backdrop-blur-md border border-white/10 rounded-2xl p-5 transition-all relative overflow-hidden hover:translate-y-[-2px] hover:bg-white/20 hover:shadow-lg"
    >
      {/* Drag Handle */}
      <div ref={setActivatorNodeRef} {...listeners} {...attributes} className="absolute top-0 right-0 p-3 cursor-grab opacity-0 hover:opacity-100 transition-opacity z-10">
